...
**AUTO** sits below the channel controls and is available only while the channel drawer is open. It observes the current channel's existing muted thumbnail, not the main player, and stops when the drawer closes. AUTO shows `[0]` until the first strong visual scene change, waits for the picture to stabilize, then shows `[1]` until the next strong change switches channels. It has no countdown or time limit. A feed that blocks thumbnail canvas inspection shows `NO FRAME`; use a timed mode for that feed. **:30** and **:60** remain in the header with a visible countdown and switch after 30 or 60 seconds of playback. Enter a whole number from 5 to 3600 in the custom field to set an interval in seconds; it also has a countdown. Click an active button again or press `S` to stop Scan. Scan does not inspect audio or understand speech, and it does not change the main stream-loading path or alter CORS settings.
