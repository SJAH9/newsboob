...
**AUTO** sits below the channel controls and is available only while the channel drawer is open. Select AUTO, click START at the beginning of a user-chosen interval, then click STOP at its end. The measured interval becomes the dwell time for each channel; the first automatic switch happens one full interval after STOP. Click START again to measure a new interval, click AUTO again to stop, or close the drawer to stop. Intervals shorter than two seconds are rejected. AUTO never inspects video or audio, and it does not change the main stream-loading path or CORS settings. **:30** and **:60** remain in the header with a visible countdown and switch after 30 or 60 seconds of playback. Enter a whole number from 5 to 3600 in the custom field to set an interval in seconds; it also has a countdown.
